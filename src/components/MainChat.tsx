...
export default function Chat(): JSX.Element {
  ...
  return (
    <div className="flex flex-col bg-green-400 h-44 flex-grow">
      {/* chat messages display*/}
      <div className="bg-yellow-300 h-5/6 overflow-y-auto overflow-x-hidden max-h-">
        {messages.map((message, index) => (
          ...
          <div key={index} className="flex m-5">
            ...
            {/* chat text */}
            <div className="bg-red-500 mx-4 w-1/2 p-2 rounded-md">{message}</div>
          </div>
        ))}
      </div>
      ...
      <form
        ...
        className="flex justify-center items-center h-1/6 bg-slate-300"
      >
        <input
